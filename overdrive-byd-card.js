class OverdriveBYDCard extends HTMLElement {
  ...
  section(title, icon, rows, open = false) {
    return `<details class="section dropdown" ${open ? "open" : ""}>
      <summary><span><ha-icon icon="${icon}"></ha-icon>${title}</span><ha-icon class="chevron" icon="mdi:chevron-down"></ha-icon></summary>
      <div class="sectionBody">${rows.map(([l, v]) => this.row(l, v)).join("")}</div>
    </details>`;
  }
  tyres() {
    return `<details class="section dropdown">
      <summary><span><ha-icon icon="mdi:car-tire-alert"></ha-icon>Tyres</span><ha-icon class="chevron" icon="mdi:chevron-down"></ha-icon></summary>
      <div class="sectionBody"><div class="tyres"><div>${this.tyre("FL", "tyre_p_fl", "tyre_t_fl")}</div><div>${this.tyre("FR", "tyre_p_fr", "tyre_t_fr")}</div><div>${this.tyre("RL", "tyre_p_rl", "tyre_t_rl")}</div><div>${this.tyre("RR", "tyre_p_rr", "tyre_t_rr")}</div></div>${this.row("System", this.value("tyre_system_state"))}${this.row("Temp State", this.value("tyre_temp_state"))}</div>
    </details>`;
  }
